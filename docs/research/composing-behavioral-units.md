...
**Object algebras** (Oliveira & Cook, "Extensibility for the Masses: Practical Extensibility with Object Algebras," ECOOP 2012): the OO twin. An algebra is an interface of constructors parameterized by the result type; an interpreter is a concrete implementation; *combining* interpreters is itself an algebra. Per the paper abstract: a solution that works "in OO languages with simple generics (including Java or C#)" without "F-bounded quantification, wildcards and variance annotations."
