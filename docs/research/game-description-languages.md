...
- **Axiom Development Kit (Schmidt, 2007)** is a Zillions plug-in that replaces the ZRF interpreter with a Forth-based engine. **Lesson:** when your minimalist DSL hits the wall, embedding a real Turing-complete scripting language is the usual escape hatch. A card DSL should provide controlled extensibility from the start.
