...
| **CARDSTOCK / ReCycle** (Bell & Goadrich, "Automated Playtesting with RECYCLEd CARDSTOCK," *Game & Puzzle Design* 2(1):71–83, 2016) | C#, with an ANTLR grammar (`CardStock/Recycle.g4`) | Declarative DSL with explicit hands/decks/zones — per github.com/mgoadric/cardstock, "There are currently 70+ games coded in RECYCLE, a mixture of classic and modern games, categorized by genre." | Generic card games; the repository holds 117 `.rcy` files directly under `games/` and 149 including subdirectories (derived from the repository tree, 2026-09-07 — upstream's own prose has lagged the count more than once) | **Yes** — closest existing prior art for a generic card-game DSL |
...
3. **Read the CARDSTOCK / RECYCLE paper** (Bell & Goadrich, *Game & Puzzle Design* 2016) and the project repo at github.com/mgoadric/cardstock — closest direct prior art, with 117 games under `games/`. Read `CardStock/Recycle.g4` first: it is the whole language in one ANTLR file, and the surface it does NOT have is as instructive as the surface it does.
...
- **CARDSTOCK/ReCycle's corpus is larger and deeper than its own README advertises, and uneven.** Hearts is present in nine variants and Cribbage is expressed in full — fifteens, pairs, runs with the runs-inside-runs subtlety, flush, his nob, his heels, 31, go and last card, in 222 lines. Bridge, Poker and Scopone sit in a `games/BustedJunk/` directory whose abandonment reason is not on the record, and 18 of the 149 files carry TODO or HACK comments, at least two of them in scoring. So it is neither "mostly small designs" nor a finished platform: read it as the best available evidence of what a declarative card DSL reaches, and read `BustedJunk/` for where one stops.
