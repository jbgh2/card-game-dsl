...
fully-kernel games (the projection substrate + general adapter; the seven
...
info-set leak (§6, §9 step 4) is closed for the seven fully-kernel games
(Seven-Card Stud joined when its showdown left `instantiate`); it remains open
for the seven `instantiate` games, as does the front-end derivation (§5).*
...
4. **Close the info-set leak as its own workstream — *done* for the seven
   ...
   Hell, Big Two, and Seven-Card Stud — proven by `tests/test_openspiel_ready.py`
   ...
   never places a bid, let alone reaches trick play; Stud's conformance is a
   bounded random API walk, its full sim being quadratic in a ~10k-action
   game). This was the payoff that justified the exercise. The seven
