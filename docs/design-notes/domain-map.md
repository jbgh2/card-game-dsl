...
| Observation | `runtime/observe.py`, `openspiel/infostate.py` | `EVENT_PAYLOADS` (each kind and the shape of every field it carries), pinned by an every-game emission sweep and refused where consumed (`payload_refusal`); renderer shape Owner Guards (undeclared value shapes refuse); the partition proof battery, whose per-field payload probes derive from the same table |
