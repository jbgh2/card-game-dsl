...
| **Merge Lane** | Who may perform a merge, decided by change class: Merge Lane A (clean-pass agent merge), Merge Lane B (reviewed agent merge), Merge Lane C (operator merge). The merge gate itself is lane-invariant. Never bare "lane". | `harness.md` |
