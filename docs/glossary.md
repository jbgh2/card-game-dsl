...
| **Architect** | Hoyle's engine-side counterpart, named **Foster** ("according to Foster"): the persona consulted on engine-structural questions (passes and Contract blocks, types, IR, runtime, diagnostics machinery, testing strategy, observability). Cites its sourcebook and principles note; standing tensions with settled law are filed, never normalized. Advises, never merges. | `harness.md` |
