...
| **Language Owner** | The persona that owns the language's taste: consulted on every Merge Lane A change and any design that would create one, supplying details — worked alternatives, precedent, corpus impact — to the operator's decision. Advises, never merges; not a Standing Role (consulted, not scheduled). Proper name and charter live in its skill. | `harness.md` |
