...
definition: The `primitives { }` game clause declaring what a [[game]] borrows from outside the DSL — one colon-row per [[primitive]], carrying its typed signature — the declared type names, and the [[collection-type]] — and its [[reads-clause]] (`pinochle_meld_value(p : Player) : Integer reads hand[p], trump_suit in hand_sequence`). Its PRESENCE partitions the game's native-call namespace in both directions: with a block, declaring is what admits a name — the game calls its own entries plus the [[builtins]], and a neighbour's Primitive is an unknown name here because THIS game did not declare it, not because it belongs to another game; without one it keeps the hand-authored `PRIMITIVE_CALL_FUNCS` registry, which admits every Primitive by name and runs none of them. An EMPTY block is a declaration ("this game borrows no Python"), not an absence.
layer: compiler
status: canonical
reserved: false
