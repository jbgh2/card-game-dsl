...
definition: The climb [[form]]'s same-seat regime — a play whose `announce` names tokens is followed by exactly one further decision of the seat that made it, over those tokens, before the ring advances; the token is announced publicly and recorded beside every play in the trick's own event record, which a game-local query reads to enforce a rule that spans plays. Tichu's Mahjong wish is the witness. Void once the round has terminated, since the hand is over. The vocabulary is the engine's, docked by `primitives.climb_announcements`.
layer: kernel
status: canonical
reserved: false
