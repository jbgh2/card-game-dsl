...
game Schnapsen {
  ...
  cards: schnapsen20
  ranking: ace-ten
  card_points { A: 11  10: 10  K: 4  Q: 3  J: 2 }
  ...
  phase hand_sequence repeat until (any player where game_score[player] <= 0) {
    ...
    before_each {
      ...
      for each player p: points_taken[p] := 0
    ...
    phase play -> outcome {
      ...
      repeat until (any player where hand[player] is empty) {
        ...
        if points_taken[w] >= 66 {
          produce claimed(w,
                          if closed_by is not none and closed_by is w then closer_opp_cp else points_taken[lo],
                          if closed_by is not none and closed_by is w then closer_opp_tr else tricks_won[lo])
        }
        if points_taken[lo] >= 66 {
          produce claimed(lo,
                          if closed_by is not none and closed_by is lo then closer_opp_cp else points_taken[w],
...
move_type declare_marriage(s : Suit) {
  ...
  effect {
    ...
    if tricks_won[actor] > 0 { points_taken[actor] += if s is trump_suit then 40 else 20 }
...
move_type close_talon {
  ...
  effect {
    ...
    closer_opp_cp := points_taken[opp]
