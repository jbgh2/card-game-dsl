  ...
  phase hand_sequence repeat until (any player where game_score[player] <= 0) {
    ...
    phase play -> outcome {
      ...
      repeat until (any player where hand[player] is empty) {
        ...
        // `for each player p: if p is fol` wrapper binds the single acting
        ...
        if endgame {
          for each player p: if p is fol {
            move chosen one card from hand[p] where follow_ok(p, card) to trick_pile
          }
        } else {
          for each player p: if p is fol {
