...
| **Resolution** | A deterministic computation over current state, used to drive non-choice moves (e.g., "who won the trick"). Who won a trick is settled by the game's declared **Trick Order** where it has one — what a trump is, what class a card follows as, how strong it is — and by the printed suits plus a `trump:` clause where it does not (see [decisions.md](decisions.md) "Trick Order"). |
| **Scoring** | Not a primitive of its own. A game scores with a declared data table (`card_points { }`), the general constructs everything else uses, and a ranking direction (`winner:` / `loser:`); see [decisions.md](decisions.md) "Scoring has no constructs of its own". |
