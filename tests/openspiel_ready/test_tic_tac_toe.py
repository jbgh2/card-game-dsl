...
- Seed insensitivity here is OVER-DETERMINED, and no single cause carries it:
  the game draws nothing at all, AND fungible marks + attribute-based setup +
  ...
  is ``shuffle box`` PLUS a position-based deal (``move 5 pieces from box to
  reserve[0]``): the x/o split becomes seed-dependent
  (``reserve[0]=[o,o,o,x,x]`` under one seed, ``[o,x,x,x,x]`` under another),
  and the sorted identity views diverge.
"""
