...
The chooser. Rather than pinning `PYTHONHASHSEED` — the obvious lever for
making a cross-process comparison reproducible — this harness sorts every
candidate list by `repr(observe.render(candidate))` before picking. `render`
is the runtime's own closed-domain, deterministic candidate rendering
...
environment one run is launched in. This is a STRONGER guarantee than
...
is therefore not pinned by this harness — a deliberate, verified choice, and
the one metamorphic-suite.md states as the suite's constraint, so the note and
the harness describe one thing.
