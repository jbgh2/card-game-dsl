...
def test_peg_origin_of_routing_round_trip() -> None:
    ...
    ctx = _peg_ctx([c for _, c in sequence], seq_bits=seq_bits, seq_len=len(sequence), dealer=1)
    play_pile = ctx.rs.zones.single("play_pile")
...
def test_peg_origin_of_requires_reading_before_the_pile_drains() -> None:
    ...
    ctx = _peg_ctx([c0, c1], seq_bits=0b01, seq_len=2, dealer=1)
    ctx.rs.zones.single("play_pile").remove(c0)
    with pytest.raises(ValueError):
        peg_origin_of(ctx, c0)
