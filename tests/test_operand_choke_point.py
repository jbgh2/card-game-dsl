...
def _assignable_calls() -> list[ast.Call]:
    return [
        node
        for node in ast.walk(_TREE)
...
def test_choke_point_holds_exactly_one_assignable() -> None:
    """`_check_operand` IS the single `assignable` call every operand routes
    ...
    inside = [c for c in _assignable_calls() if _in_choke_point(c)]
    assert len(inside) == 1
