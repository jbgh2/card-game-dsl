...
def test_rejects_produce_outside_an_outcome_phase() -> None:
    # `produce` is only meaningful in an outcome-phase body; elsewhere it
    # raises an uncaught signal at runtime, so it must be a compile error.
    src = _game("score[player] : Integer = 0", "produce won")
    with pytest.raises(DiagnosticError) as ei:
        check_dsl(src, "g.cardlang")
    assert "'produce' may only appear in an outcome-phase body" in str(ei.value)


def test_rejects_dot_form_on_a_player_receiver() -> None:
    # The dot form is object-member access only (Card, Move).
...
def test_every_type_union_member_is_classified_by_the_member_arm() -> None:
    ...
    field-checking arm (like `TCard`), classify it as a reject, or
...
# completeness"; the TCard/collection positives above are the negative
