...
def test_pinochle_space_adds_the_bid_and_trump_vocabulary() -> None:
    ...
    # 52 cards, then the integer block the bid ladder declares — 0..150, the
    # bid in TENS up to the game's own 1500 ceiling — then the move names in
    ...
    assert space.num_distinct_actions == 52 + 151 + 10
    assert [space.to_string(a) for a in (52, 202)] == ["0", "150"]
    assert [space.to_string(a) for a in range(203, 213)] == [
        "submit_bid",
        "pass",
        "pass_with_help",
...
def test_existing_games_keep_the_standard_52_card_block() -> None:
    ...
    assert _space("pinochle.cardlang").num_distinct_actions == 213  # unchanged
