...
description: "Use when writing or rewriting ANY pull-request description in this repo. States problem and fix at a high level, no mechanism, aggressively terse — while keeping the two artifacts the review gates read: the Claims line and (when needed) the class ledger. The diff shows the mechanism; the tracker holds the findings; the ledgers hold the completeness record. The description frames."
---
...
delta named in one line | a stated negative | goldens regenerated because …>
**Artifacts:** <grid/ledger module paths, named not restated; the class
ledger block only when the class is not already derived in a citable
artifact>
...
- A claim may be a NEGATIVE — "no front-end, runtime, or golden change" —
  and a whole-diff negative is often the claim most worth stating: it is
  the one the merge-base replay falsifies cheapest, so stating it invites
  exactly the skepticism it deserves.
...
- **The class ledger keys on derivation, not provenance.** The block
  (surface-totality-audit Step 2b, verbatim form) appears when the change
  answers a finding whose class is NOT yet derived anywhere citable — the
  raw review finding arriving mid-loop. When a tracker issue or module
  ledger already IS the derivation, cite it instead: a third copy drifts
  (maintaining.md rule 4).
- The Claims line and any required class ledger are exempt from terseness —
  they are artifacts the machinery reads, not prose.
...
> class derived in #95 — cited, not restated.
>
